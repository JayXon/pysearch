...
type CacheLevel = DashMap<Vector, Expr>;
...
fn save(level: &CacheLevel, output: Vector, expr: Expr) {
...
fn find_expressions(cache: &mut Cache, n: usize) {
    ...
    (1..n).into_par_iter().for_each(|k| {
        cache[k].par_iter().for_each(|(or, er)| {
            // 1-byte operators
            if n >= k + 2 {
    ...
    if n >= 3 {
        cache[n - 2].par_iter().for_each(|(or, er)| {
    ...
    if n >= 2 {
        cache[n - 1].par_iter().for_each(|(or, er)| {
            let erp: NonNull<Expr> = er.into();
            if er.prec() >= 12 {
                if USE_BIT_NEG {
                    save(&cn, !or.clone(), Expr::unary(erp, Operator::BitNeg));
                }
                if USE_NEG {
                    save(&cn, -or.clone(), Expr::unary(erp, Operator::Neg));
                }
            }
        });
    }
    cache.push(cn.into_par_iter().collect());
}
